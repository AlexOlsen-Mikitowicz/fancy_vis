...
tm_shape(co) + 
  ...
  tm_shape(sf_p) + 
  tm_dots(col='station',size=3,palette=c('red','white', "blue"))
...
precip_long <- daily_p %>%
  gather(., "measurement", "value", c(5:7))

slice_q1_temp <- precip_long %>%
  group_by(station)

q1_temp <- ggplot(slice_q1_temp, aes(x=date, y = value, color = measurement, group = station)) +
  geom_line()+
  geom_point()+
  facet_wrap(~station)+
  transition_reveal(date)+
...
animate(q1_temp, nframes = nrow(slice_q1_temp))
...
mil_stations <- c('AFF', 'BKF', "FCS", "SHM")

#Download data from CO front range military bases

buck_afb <- riem_measures(station='BKF',date_start = '2018-10-01',
                      date_end = '2019-09-30')

fort_cars <- riem_measures(station='FCS',date_start = '2018-10-01',
                      date_end = '2019-09-30')

af_acad <- riem_measures(station='AFF',date_start = '2018-10-01',
                      date_end = '2019-09-30')

#Merge datasets into one and create temperature min max and mean - create tibble

military_daily_p <- af_acad %>% 
  bind_rows(af_acad,
            buck_afb,
            fort_cars) %>%
  mutate(date = as.Date(valid)) %>%
...
#Make military bases spatial points

military_daily_sf <- military_daily_p %>%
  st_as_sf(.,coords=c('lon','lat'),crs=4326,remove=F)



military_full_dates <- unique(military_daily_sf$date)

#use for end loop: 

military_sequence <- 1:length(military_full_dates)
saveGIF({
  for(i in mil_sequence){
  military_sub_sf <- military_daily_sf %>%
    filter(date == military_full_dates[i]) %>%
    filter(daily_p > 0)


  if(nrow(mil_sub_sf) > 0){
  mililitary_finalmap <- basemap + tm_shape(military_sub_sf) + 
    tm_dots(title.size='Daily Precip @ Military Bases (in)',
            col='lon',
            size='daily_p',
            style='cont',
            palette='Oranges',
            ...
            scale=3.5,
            sizes.legend=c(0,1,5,10,20))
  }else{military_finalmap <- basemap}
  print(military_finalmap + 
          tm_layout(title=military_full_dates[i],
        legend.position=c(0.7,0.2)))
  }
},movie.name = 'precip_flash_military_bases.mp4',
  interval=0.12,
  ani.width=1000,
  ani.height=800)
